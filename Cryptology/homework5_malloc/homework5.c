...
void decryptNumber(char **);
int decryptAuxiliaries(char *, size_t );
int checkCorruption(char *, char **, char *, char *);

int main(void) {
    // initializing required variables
    size_t i, j; // counters
    char *name = (char *)malloc(sizeof(char) * NAME_SIZE);  // a matrix to keep the name
    char **number = (char **)malloc(sizeof(char *) * NUM_SIZE);  // a 2D matrix to keep the number
    for(i = 0; i < NUM_SIZE; i++) {
        *(number+i) = (char *)malloc(sizeof(char) * NUM_SIZE);
    }
    char *mode_b = (char *)malloc(sizeof(char) * MODE_SIZE);  // a matrix to keep the mode
    char *modifier_b = (char *)malloc(sizeof(char) * MODIFIER_SIZE);  // a matrix to keep the modifier
    int mode, modifier;
    ...
    if(checkCorruption(name, number, mode_b, modifier_b)) { // checking if the input is valid to decrypt
        ...
        free(name);  // deleting memory
        for(i = 0; i < NUM_SIZE; i++) {
            free(*(number+i));  // deleting memory
        }
        free(number);  // deleting memory
        free(mode_b);  // deleting memory
        free(modifier_b);  // deleting memory
        return 1;
    }
    else {
...
void decryptName(char *name, int mode, int modifier) {
    char *temp_name = (char *)malloc(sizeof(char) * NAME_SIZE);  // creating temporary matrix to hold the matrix's value
    size_t i;  // counter
    switch(mode) {
    ...
    free(temp_name);  // deleting memory
}
// function that decrypts the number
void decryptNumber(char **number) {
    size_t i, j;  // counters
    char **temp_number = (char **)malloc(sizeof(char *) * NUM_SIZE);  // creating temporary matrix to hold values of original matrix
    for(i = 0; i < NUM_SIZE; i++) {
        *(temp_number+i) = (char *)malloc(sizeof(char) * NUM_SIZE);
    }
    for(i = 0; i < NUM_SIZE; i++) {
        for(j = 0; j < NUM_SIZE; j++) {
    ...
    for(i = 0; i < NUM_SIZE; i++) {
        free(*(temp_number+i));  // deleting memory
    }
    free(temp_number);  // deleting memory
}
...
int checkCorruption(char *name, char **number, char *mode_b, char *modifier_b) {
