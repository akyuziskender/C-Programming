    ...
    do {
        ...
        switch(option) {
            ...
            case '2':
				...
				if(strlen(word) < 3) {
					puts("Error: You did not enter at least 3 characters. Try again.");
				}
				else {
...
void search(Movies *movies, char word[]) {
    ...
    for(i = 0; i < num_of_movies; i++) {
        ...
        if( strstr(temp, temp2) != NULL) {  // checking if there is a match
            printf("   %d. %*s %d %s %.1f %s ", j, -max_length(movies), movies[i].name,
            movies[i].year, movies[i].country, movies[i].rating,
            movies[i].from_book);
            if(strcmp(movies[i].from_book, "YES") == 0) {
                printf("%s %s %d\n", movies[i].book.book_name, movies[i].book.book_writer, movies[i].book.book_year);
            }
            else {
                puts(""); // new line
            }
...
void top_movies(Movies *movies, int n) {
    ...
    int max_len = max_length(movies);  // getting the length of the longest movie name
...
// a function that checks if the movie which user wants to insert is already in the database
